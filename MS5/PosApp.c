...
void addItem(void) {
    ...
    if (noOfItems < MAX_NO_ITEMS) {
        ...
        printf("Price: ");
        items[noOfItems].price = getDbl();
        printf("Is the item Taxed? ");
        items[noOfItems].taxed = yes();
        printf("Quantity: ");
        items[noOfItems].quantity = getIntMM(1, MAX_STOCK_NUMBER, "Quantity");
        noOfItems++;
        start("Done!");
    }
    else {
...
void removeItem(void) {
    start("Remove Item");
    int i;
    for (i = selectItems() - 1; i < noOfItems; i++) {
        items[i] = items[i + 1];
    }
    noOfItems--;
    start("Done!");
...
void stockItem(void) {
    start("Stock Items");
    int row = selectItems() - 1;
    display(&items[row]);
    printf("Quantity to add: ");
    items[row].quantity += getIntMM(1, MAX_STOCK_NUMBER - items[row].quantity, "Quantity to Add");
    start("Done!");
}
void POS(void) {
    ...
    double total = 0;
    start("Point Of Sale");
    while (i < MAX_BILL_ITEMS && (sret = search()) != -2) {
        if (sret >= 0 && items[sret].quantity > 0) {
            items[sret].quantity--;
            display(&items[sret]);
            bill[i] = &items[sret];
            i++;
        }
        else if (sret >= 0 && items[sret].quantity == 0) {
...
int selectItems(void) {
    ...
    listItems();
    printf("Select row: ");
    return ret = getIntMM(1, noOfItems, "Row Number");
}
